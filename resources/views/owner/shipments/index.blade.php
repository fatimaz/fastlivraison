              ...
              <tbody>
                   @isset($shipments)
                      ...
                      <tr>
                       <td>{{ $key +1 }}</td>
                       <!-- <td>{{$shipment -> name}}</td> -->
                       <!-- <td>{{$shipment -> countries->name}}</td>
                       <td>{{$shipment -> countriesto->name}}</td> -->
                        <td>{{$shipment -> user-> name}}</td>
                        <td>{{$shipment ->  price}}</td>
                        <td>{{$shipment ->  qty}}</td>
                        <td> <img style="width: 150px; height: 100px;" src="{{$shipment -> photo }}"></td>
                        <td>{{$shipment ->  deleted_at}}</td>
                      <td>{{$shipment -> getActive()}}</td>
                  <td>
                  <a href="{{route('admin.shipments.show',$shipment -> id)}}" class="btn btn-sm btn-info">Show</a>
